...
#ifndef STATE_MACHINE_COOPERATIVE_H_
#define STATE_MACHINE_COOPERATIVE_H_
...
namespace state_machine_cooperative {
...
{
  EncoderUpdate(const Eigen::Vector3d& _encoder)
      : encoder(_encoder)
  {
  }

  Eigen::Vector3d encoder;
};

struct BackToPositionHold {};
struct Takeoff {};
struct OdometryWatchdog {};
struct EncoderWatchdog {};
...
typedef boost::msm::back::state_machine<StateMachineDefinition, boost::msm::back::mpl_graph_fsm_check> StateMachineCooperative;
...
class StateMachineDefinition : public msm_front::state_machine_def<StateMachineDefinition>
{
 private:
  ...
  struct PrintEncoderWatchdogWarning;
 ...
 public:
  ...
  struct transition_table : boost::mpl::vector<
      ...
      msm_front::Row<Inactive, EncoderWatchdog, InternalTransition, PrintEncoderWatchdogWarning, EncoderOutdated >,      
      msm_front::Row<Inactive, EncoderUpdate, InternalTransition, SetEncoder, NoGuard >,      
      ...
      msm_front::Row<RemoteControl, EncoderWatchdog, InternalTransition, PrintEncoderWatchdogWarning, EncoderOutdated >,      
      //  +---------+-------------+---------+---------------------------+----------------------+
      ...
      msm_front::Row<RemoteControlReadyForOdometry, EncoderUpdate, InternalTransition, SetEncoder, NoGuard >,
      msm_front::Row<RemoteControlReadyForOdometry, OdometryWatchdog, RemoteControl, PrintOdometryWatchdogWarning, OdometryOutdated >,
      msm_front::Row<RemoteControlReadyForOdometry, EncoderWatchdog, RemoteControl, PrintEncoderWatchdogWarning, EncoderOutdated >, 
      //  +---------+-------------+---------+---------------------------+----------------------+
      ...
      msm_front::Row<HaveOdometry, EncoderUpdate, InternalTransition, SetEncoder, NoGuard >,
      msm_front::Row<HaveOdometry, OdometryWatchdog, RemoteControl, PrintOdometryWatchdogWarning, OdometryOutdated >,
      msm_front::Row<HaveOdometry, EncoderWatchdog, RemoteControl, PrintEncoderWatchdogWarning, EncoderOutdated >,      
      ...
      msm_front::Row<PositionHold, EncoderUpdate, InternalTransition, SetEncoder, NoGuard>,
      ...
      msm_front::Row<PositionHold, EncoderWatchdog, RemoteControl, PrintEncoderWatchdogWarning, EncoderOutdated >,      
      //  +---------+-------------+---------+---------------------------+----------------------+
      ...
      msm_front::Row<RcTeleOp, EncoderUpdate, InternalTransition, SetEncoder, NoGuard>,
      msm_front::Row<RcTeleOp, OdometryWatchdog, RemoteControl, PrintOdometryWatchdogWarning, OdometryOutdated >,
      msm_front::Row<RcTeleOp, EncoderWatchdog, RemoteControl, PrintEncoderWatchdogWarning, EncoderOutdated >
      >
  {
  };
...
private:
  ...
  std::shared_ptr<PositionControllerInterfaceCooperative> controller_;
  ros::Publisher command_publisher_quad1, command_publisher_quad2;
  ...
  Eigen::Vector3d current_encoder1_;
  int64_t current_enc1_timestamp_ns;
  mav_msgs::EigenTrajectoryPointDeque current_reference_queue_;

  void PublishAttitudeThrustCommand (const mav_msgs::EigenAttitudeThrust& command_quad1, 
      const mav_msgs::EigenAttitudeThrust& command_quad2) const;
  ...
  struct SetReferenceAttitude
  {
    ...
    void operator()(const RcUpdate& evt, FSM& fsm, SourceState&, TargetState&)
    {
      ...
      // fsm.PublishThrustCommand(command);

      mav_msgs::EigenAttitudeThrust command_quad1, command_quad2;
      command_quad1.thrust.x() = 0;
      command_quad1.thrust.y() = 0;
      command_quad1.thrust.z() = fsm.controller_->getMass1() * 9.81;
      command_quad1.attitude = Eigen::AngleAxisd(0, Eigen::Vector3d::UnitZ());\

      command_quad2.thrust.x() = 0;
      command_quad2.thrust.y() = 0;
      command_quad2.thrust.z() = fsm.controller_->getMass2() * 9.81;
      command_quad2.attitude = Eigen::AngleAxisd(0, Eigen::Vector3d::UnitZ());

      fsm.PublishAttitudeThrustCommand(command_quad1, command_quad2);      
    }
  };
  ...
  struct SetOdometry
  {
    ...
    void operator()(const OdometryUpdate& evt, FSM& fsm, SourceState&, TargetState&)
    {
      ...
      fsm.controller_->setOdometry(evt.odometry, &fsm.current_state_);
      //ROS_INFO_STREAM_THROTTLE(1,"odom time " << fsm.current_state.timestamp_ns);
    }
  };
  ...
  {
    template<class FSM, class SourceState, class TargetState>
    void operator()(const EncoderUpdate& evt, FSM& fsm, SourceState&, TargetState&)
    {
      fsm.current_encoder1_ = evt.encoder;
      fsm.current_enc1_timestamp_ns = ros::Time::now().toNSec();
      fsm.controller_->setEncoder(evt.encoder);
    }
  };  
  ...
  struct ComputeCommand
  {
    ...
    {
      mav_msgs::EigenAttitudeThrust command_quad1, command_quad2;
      //ROS_INFO("ComputeCommand");
      fsm.controller_->calculateAttitudeThrustCommandCentralize(&command_quad1, &command_quad2);
      fsm.PublishAttitudeThrustCommand(command_quad1, command_quad2);
...
//      }
  ...
  struct PrintEncoderWatchdogWarning
  {
    ...
    {
      if (!fsm.use_rc_teleop_) {
        ROS_WARN_STREAM("No ENCODER message received in the last "<< kOdometryOutdated_ns/1000000000.0 << " seconds!");
      } else {
        ROS_WARN_STREAM(
            "No ENCODER message received in the last "<< kOdometryOutdated_ns/1000000000.0 << " seconds!"
            " -- going back to manual remote control");
      }
    }
  };  
  ...
  {
    template<class FSM, class SourceState, class TargetState>
    bool operator()(const EncoderWatchdog& evt, FSM& fsm, SourceState&, TargetState&)
    {
      return std::abs(static_cast<int64_t>(ros::Time::now().toNSec()) - fsm.current_enc1_timestamp_ns) > kOdometryOutdated_ns;
    }
  };
