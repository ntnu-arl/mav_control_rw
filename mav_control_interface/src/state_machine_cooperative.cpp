...
#include "state_machine_cooperative.h"
...
{
  std::string quad1_cmd_topic;
  nh.param<std::string>("quad1_cmd_topic", quad1_cmd_topic, "/quad1/command/attitude_thrust");
  std::cout << "quad1 cmd topic: " << quad1_cmd_topic << '\n';

  std::string quad2_cmd_topic;
  nh.param<std::string>("quad2_cmd_topic", quad2_cmd_topic, "/quad2/command/attitude_thrust");
  std::cout << "quad2 cmd topic: " << quad2_cmd_topic << '\n';    

  command_publisher_quad1 = nh_.advertise<mav_msgs::AttitudeThrust>(
      quad1_cmd_topic, 1);

  command_publisher_quad2 = nh_.advertise<mav_msgs::AttitudeThrust>(
      quad2_cmd_topic, 1);
...
void StateMachineDefinition::PublishAttitudeThrustCommand (
    const mav_msgs::EigenAttitudeThrust& command_quad1, const mav_msgs::EigenAttitudeThrust& command_quad2) const
{
  ...
  mav_msgs::AttitudeThrustPtr msg_quad1(new mav_msgs::AttitudeThrust);
  mav_msgs::AttitudeThrustPtr msg_quad2(new mav_msgs::AttitudeThrust);

  mav_msgs::EigenAttitudeThrust tmp_command_quad1 = command_quad1;
  tmp_command_quad1.thrust.x() = 0;
  tmp_command_quad1.thrust.y() = 0;
  tmp_command_quad1.thrust.z() = std::max(0.0, command_quad1.thrust.z());

  mav_msgs::EigenAttitudeThrust tmp_command_quad2 = command_quad2;
  tmp_command_quad2.thrust.x() = 0;
  tmp_command_quad2.thrust.y() = 0;
  tmp_command_quad2.thrust.z() = std::max(0.0, command_quad2.thrust.z());

  msg_quad1->header.stamp = ros::Time::now();  // TODO(acmarkus): get from msg
  mav_msgs::msgAttitudeThrustFromEigen(command_quad1, msg_quad1.get());
  msg_quad2->header.stamp = msg_quad1->header.stamp;
  mav_msgs::msgAttitudeThrustFromEigen(command_quad2, msg_quad2.get());

  command_publisher_quad1.publish(msg_quad1);
  command_publisher_quad2.publish(msg_quad2);
}
