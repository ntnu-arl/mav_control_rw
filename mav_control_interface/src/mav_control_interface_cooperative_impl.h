...
#ifndef MAV_CONTROL_INTERFACE_COOPERATIVE_IMPL_H
#define MAV_CONTROL_INTERFACE_COOPERATIVE_IMPL_H
...
class MavControlInterfaceCooperativeImpl
{
 public:
  MavControlInterfaceCooperativeImpl(ros::NodeHandle& nh, ros::NodeHandle& private_nh,
                          std::shared_ptr<PositionControllerInterfaceCooperative> controller,
                          std::shared_ptr<RcInterfaceBase> rc_interface);

  virtual ~MavControlInterfaceCooperativeImpl();
 ...
 private:
  ...
  std::unique_ptr<state_machine_cooperative::StateMachineCooperative> state_machine_;
  ...
  void EncoderCallback(const geometry_msgs::Vector3::ConstPtr& msg);
  void OdometryWatchdogCallback(const ros::TimerEvent& e);
  void EncoderWatchdogCallback(const ros::TimerEvent& e);
  ...
  void publishAttitudeThustCommand(const mav_msgs::AttitudeThrust& command_quad1, const mav_msgs::AttitudeThrust& command_quad2);
};
