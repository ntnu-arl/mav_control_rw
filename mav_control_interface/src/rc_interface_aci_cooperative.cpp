...
RcInterfaceAciCooperative::RcInterfaceAciCooperative(const ros::NodeHandle& nh)
    : RcInterfaceBase(),
      nh_(nh),
      is_on_(false)
{
  rc_sub_ = nh_.subscribe("rc", 1, &RcInterfaceAciCooperative::rcCallback, this);
}

void RcInterfaceAciCooperative::rcCallback(const sensor_msgs::JoyConstPtr& msg)
{
  is_on_ = isRcOn(msg);
...
std::string RcInterfaceAciCooperative::getName() const
{
  return std::string("ACI rc interface cooperative");
}

RcData RcInterfaceAciCooperative::getRcData() const
{
  return last_data_;
}

bool RcInterfaceAciCooperative::isActive() const
{
  if (!is_on_)
    return false;
...
bool RcInterfaceAciCooperative::isOn() const
{
  return is_on_;
}

float RcInterfaceAciCooperative::getStickDeadzone() const
{
  return STICK_DEADZONE;
}

bool RcInterfaceAciCooperative::isRcOn(const sensor_msgs::JoyConstPtr& msg) const
{
